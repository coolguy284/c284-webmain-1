...
async function createSitemap(sites) {
  ...
  await fs.promises.writeFile(
    ...
    (await Promise.all(
      Array.from(sites.entries()).map(async site =>
        ...
        `    <lastmod>${(await fs.promises.stat('websites/public' + site[0])).mtime.toISOString().slice(0, -1)}+00:00</lastmod>\n` +
...
if (require.main == module) {
  ...
  crawler.crawl('/index.html', crawler.fsGetterFuncGen('websites/public'))
    .then(createSitemap);
} else {
